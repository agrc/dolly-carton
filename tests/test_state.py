...
class TestIntegration:
    ...
    def test_set_table_hash_updates_nested_map_without_overwrite(self, monkeypatch):
        monkeypatch.setenv("APP_ENVIRONMENT", "prod")
        # Write two different tables; ensure both survive
        set_table_hash("SGID.Test.Table1", "h1")
        set_table_hash("SGID.Test.Table2", "h2")

        hashes = get_table_hashes()
        # Normalize expectation
        assert hashes["sgid.test.table1"] == "h1"
        assert hashes["sgid.test.table2"] == "h2"

        # Overwrite one and ensure the other remains
        set_table_hash("SGID.Test.Table1", "h1b")
        hashes = get_table_hashes()
        assert hashes["sgid.test.table1"] == "h1b"
        assert hashes["sgid.test.table2"] == "h2"
